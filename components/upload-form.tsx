...
export function UploadForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setUploadProgress({
      stage: 'uploading',
      message: 'Uploading file and creating transcription job...',
      progress: 10
    })
    ...
    try {
      ...
      // Submit to the API route which handles both database creation and queue submission
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to submit transcription job: ${errorText}`)
      }
      ...
      // The API route handles queue submission internally
      // We need to poll the transcription job status instead
      await pollTranscriptionJobStatus(result.transcriptionId)
  ...
  // Poll transcription job status from database
  const pollTranscriptionJobStatus = async (transcriptionId: string) => {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        setUploadProgress(prev => ({
          ...
          message: status === 'completed' ? 'Transcription completed! Auto-analyzing for risks...' :
        ...
        if (status === 'completed') {
          // Wait a moment for risk detection to start, then navigate
          setTimeout(() => {
    ...
    setUploadProgress(prev => ({
      ...prev,
      stage: 'failed',
      message: 'Polling timeout - please check transcriptions page',
      progress: 0
    }))
    setIsSubmitting(false)
  }
